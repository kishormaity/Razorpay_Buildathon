...
def main():
    print("=" * 70)
    print("         IEEE-CIS LIGHTGBM TRANSACTION + BEHAVIORAL MODEL")
    print("=" * 70)
    ...
    model_save_path = os.path.join(processed_dir, 'models/behavioral_lgb_model.txt')
    preds_parquet_path = os.path.join(processed_dir, 'predictions/behavioral_predictions.parquet')
    report_path = os.path.join(processed_dir, 'reports/behavioral_evaluation_report.md')
    ...
    print(f"  * Baseline predictions validation subset: {baseline_df.shape}")
    ...
    # 3. Assert Strict ID Sequence Alignment
    ...
    print("\n[4/7] Training LightGBM model with transaction + behavioral features...")
    ...
    print(f"  * PR-AUC (AP):    Baseline={b_pr_auc:.5f} -> Behavioral={pr_auc:.5f} (Delta={pr_auc - b_pr_auc:+.5f})")
    print(f"  * ROC-AUC:        Baseline={b_roc_auc:.5f} -> Behavioral={roc_auc:.5f} (Delta={roc_auc - b_roc_auc:+.5f})")
    print(f"  * Best F1-Score:  Baseline={b_best_f1:.5f} -> Behavioral={best_f1:.5f} (Delta={best_f1 - b_best_f1:+.5f})")

    # 7. Generate Comparative PR Curve Plot
    print("\n[6/7] Generating comparative precision-recall curve graph...")
    plt.figure(figsize=(10, 8))
    plt.plot(b_recalls, b_precisions, color='gray', linestyle='--', lw=2, label=f'Baseline (Tx-Only) (PR-AUC = {b_pr_auc:.5f})')
    plt.plot(recalls, precisions, color='darkorange', linestyle='-', lw=2.5, label=f'Behavioral (Tx + Behavior) (PR-AUC = {pr_auc:.5f})')
    plt.axvline(x=r_opt, color='blue', linestyle=':', alpha=0.8, label=f'Behavioral Opt F1 Recall = {r_opt:.3f}')
    plt.axhline(y=p_opt, color='blue', linestyle=':', alpha=0.8, label=f'Behavioral Opt F1 Precision = {p_opt:.3f}')
    plt.xlabel('Recall')
    plt.ylabel('Precision')
    plt.title('Comparative Precision-Recall Curves (Baseline vs. Behavioral)')
    ...
    print("\n[7/7] Writing behavioral evaluation report...")
    ...
    report_content = f"""# Phase 4: LightGBM Transaction + Behavioral Model Evaluation Report
...
* **Baseline (TN / FP / FN / TP)**: {btn_05:,} / {bfp_05:,} / {bfn_05:,} / {btp_05:,}
* **Behavioral (TN / FP / FN / TP)**: {tn_05:,} / {fp_05:,} / {fn_05:,} / {tp_05:,}

### Optimal Threshold
* **Baseline (TN / FP / FN / TP @ {b_best_threshold:.4f})**: {btn_opt:,} / {bfp_opt:,} / {bfn_opt:,} / {btp_opt:,}
* **Behavioral (TN / FP / FN / TP @ {best_threshold:.4f})**: {tn_opt:,} / {fp_opt:,} / {fn_opt:,} / {tp_opt:,}
...
"""
    ...
    print("\n" + "=" * 70)
    print("BEHAVIORAL MODEL TRAINING AND COMPARATIVE EVALUATION COMPLETED")
    print("=" * 70)
