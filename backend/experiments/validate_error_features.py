...
def main():
    print("=" * 70)
    print("      IEEE-CIS PHASE 10C: ERROR-DRIVEN FEATURES ABLATION & VALIDATION")
    print("=" * 70)

    current_dir = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..'))
    processed_dir = os.path.join(current_dir, 'data', 'processed')
    
    features_path = os.path.join(processed_dir, 'features/error_features.parquet')
    d_preds_8020_path = os.path.join(processed_dir, 'predictions/historical_predictions.parquet')
    report_path = os.path.join(processed_dir, 'reports/error_feature_validation_report.md')

    if not os.path.exists(features_path):
        print(f"[ERROR] Error features file not found at: {features_path}")
        sys.exit(1)

    start_time = time.time()
    df = pd.read_parquet(features_path)
    print(f"Loaded error features dataset: {df.shape} in {time.time() - start_time:.2f}s")
    ...
    # Exclude all graph base/refined features and Phase 10 new features
    graph_base_cols = [
    ...
    error_features = [
        'device_missing_email_present', 'device_missing_value_weight',
        'short_gap_velocity_ratio', 'velocity_value_drain'
    ]
    
    all_excluded = (core_deviations + diversity_cols + metadata_cols + 
                    graph_base_cols + graph_refined_cols + error_features)
    ...
    # -------------------------------------------------------------
    print("\n" + "=" * 50)
    print("STAGE 1: Running Ablation on 80/20 Split (E1 - E4)")
    print("=" * 50)
    ...
    configs = {
        'E1 (Device Missing & Email Present)': ['device_missing_email_present'],
        'E2 (Device Missing Value Weight)': ['device_missing_value_weight'],
        'E3 (Short Gap Velocity Ratio)': ['short_gap_velocity_ratio'],
        'E4 (Velocity Value Drain)': ['velocity_value_drain']
    }
    ...
    # Identify top 2 features to combine into E5
    ...
    # E5: Combined model
    print(f"\nTraining E5 (Best 2 Combined: {best_features_to_combine})...")
    X_train_e5 = df.iloc[:split_80][base_features + best_features_to_combine]
    X_val_e5 = df.iloc[split_80:][base_features + best_features_to_combine]
    
    metrics_e5, probs_e5 = train_eval_config("E5 (Best 2 Combined)", X_train_e5, y_train_80, X_val_e5, y_val_80, categorical_cols_80)
    df_80 = pd.concat([df_80, pd.DataFrame([{'name': 'E5 (Best 2 Combined)', 'num_features': X_train_e5.shape[1], 'delta_d': metrics_e5['pr_auc'] - metrics_d_80['pr_auc'], **metrics_e5}])], axis=0).reset_index(drop=True)
    probs_map_80['E5'] = probs_e5
    ...
    best_key_80 = 'E5' if best_cand_name == 'E5 (Best 2 Combined)' else best_cand_name
    probs_map_80 = {
    ...
    stability_configs['E5 (Best 2 Combined)'] = best_features_to_combine
    ...
    if is_better_both:
        ...
        probs_key_80 = 'E5' if best_cand_name == 'E5 (Best 2 Combined)' else best_cand_name
        probs_key_70 = 'E5' if best_cand_name == 'E5 (Best 2 Combined)' else best_cand_name
        ...
        verdict = f"MODEL D REMAINS CHAMPION: Model D remains the overall pipeline champion at PR-AUC `{d_pr_80:.5f}` (80/20) and `{d_pr_70:.5f}` (70/30). No error-driven configuration consistently outpaced it."
...
"""
    ...
    report_content = f"""# Phase 10C: Targeted Feature Validation Report
...
"""
    ...
    print("Targeted feature validation report written successfully!")
