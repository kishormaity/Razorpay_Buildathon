...
def main():
    print("=" * 70)
    print("         IEEE-CIS LIGHTGBM TRANSACTION + TOP BEHAVIORAL MODEL (C)")
    print("=" * 70)
    ...
    model_b_preds_path = os.path.join(processed_dir, 'predictions/behavioral_predictions.parquet')
    
    model_save_path = os.path.join(processed_dir, 'models/top_behavioral_lgb_model.txt')
    preds_parquet_path = os.path.join(processed_dir, 'predictions/top_behavioral_predictions.parquet')
    report_path = os.path.join(processed_dir, 'reports/top_behavioral_evaluation_report.md')
    pr_curve_path = os.path.join(processed_dir, 'plots/comparative_pr_curve.png')

    # 1. Load Datasets
    print("\n[1/7] Loading behavioral features...")
    if not os.path.exists(features_path):
        print(f"[ERROR] Behavioral features parquet file not found at: {features_path}")
    ...
    if not os.path.exists(model_b_preds_path):
        print(f"[ERROR] Model B predictions file not found at: {model_b_preds_path}")
        print("Please run train_behavioral_model.py first.")
        sys.exit(1)
        
    start_time = time.time()
    df = pd.read_parquet(features_path)
    baseline_df = pd.read_parquet(baseline_preds_path)
    model_b_df = pd.read_parquet(model_b_preds_path)
    print(f"Loaded datasets in {time.time() - start_time:.2f} seconds.")
    print(f"  * Behavioral dataset: {df.shape}")
    ...
    # Verify ID Alignment
    print("Checking alignment with baseline and Model B validation sets...")
    val_ids = df.iloc[split_idx:]['TransactionID'].values
    baseline_ids = baseline_df['TransactionID'].values
    model_b_ids = model_b_df['TransactionID'].values
    
    if not np.array_equal(val_ids, baseline_ids) or not np.array_equal(val_ids, model_b_ids):
        raise ValueError("Validation set TransactionIDs do not align across baseline and behavioral models!")
    print("✅ Validation TransactionIDs align perfectly across all datasets.")
    
    # Group features matrix and labels
    # We drop the metadata, target, and the 6 lower-ranked behavioral features
    b_features_to_drop = [
        'card_tx_count_10m',
        'card_tx_count_1h',
        'card_spend_sum_1h',
    ...
    # Drop columns
    X = df.drop(columns=['TransactionID', 'isFraud'] + b_features_to_drop)
    y = df['isFraud']
    ...
    print(f"  * Model C Features Count:   {len(X_train.columns)} columns (393 Tx + 6 Top-ranked Behavior)")
    ...
    # 5. Model Training (Exact Same Parameters as Baseline and Model B)
    print("\n[4/7] Training LightGBM model with top-6 importance-ranked behavioral features...")
    ...
    print(f"Model C training completed in {time.time() - t0:.2f} seconds.")
    print(f"Best iteration: {model.best_iteration}")

    # 6. Evaluate Predictions & Compare Models A, B, C
    ...
    # Retrieve Model A (baseline) and Model B (full behavioral) metrics
    # Model A
    ...
    # Model B
    model_b_probs = model_b_df['fraud_probability'].values
    mb_pr_auc = average_precision_score(y_val, model_b_probs)
    mb_roc_auc = roc_auc_score(y_val, model_b_probs)
    
    mb_preds_05 = (model_b_probs >= 0.50).astype(int)
    mbp_05, mbr_05, mbf_05, _ = precision_recall_fscore_support(y_val, mb_preds_05, average='binary')
    mbtn_05, mbfp_05, mbfn_05, mbtp_05 = confusion_matrix(y_val, mb_preds_05).ravel()
    
    mb_precisions, mb_recalls, mb_thresholds = precision_recall_curve(y_val, model_b_probs)
    mb_f1_scores = np.divide(
        2 * mb_precisions * mb_recalls,
        mb_precisions + mb_recalls,
        out=np.zeros_like(mb_precisions),
        where=(mb_precisions + mb_recalls) > 0
    )
    mb_best_idx = np.argmax(mb_f1_scores)
    mb_best_threshold = mb_thresholds[mb_best_idx] if mb_best_idx < len(mb_thresholds) else 0.5
    mb_best_f1 = mb_f1_scores[mb_best_idx]
    mb_preds_opt = (model_b_probs >= mb_best_threshold).astype(int)
    mbp_opt, mbr_opt, mbf_opt, _ = precision_recall_fscore_support(y_val, mb_preds_opt, average='binary')
    mbtn_opt, mbfp_opt, mbfn_opt, mbtp_opt = confusion_matrix(y_val, mb_preds_opt).ravel()
    mbfpr_opt = mbfp_opt / (mbfp_opt + mbtn_opt) if (mbfp_opt + mbtn_opt) > 0 else 0.0

    print("\nCROSS-MODEL METRICS COMPARISON (A vs B vs C):")
    print(f"  * Model A (Tx-Only) PR-AUC:      {b_pr_auc:.5f}")
    print(f"  * Model B (Tx + All Behav) PR-AUC: {mb_pr_auc:.5f}")
    print(f"  * Model C (Tx + Top-6 Behav) PR-AUC: {pr_auc:.5f} (Delta vs A: {pr_auc - b_pr_auc:+.5f})")
    ...
    plt.plot(mb_recalls, mb_precisions, color='blue', linestyle=':', lw=2, label=f'Model B (Tx + All Behavior) (PR-AUC = {mb_pr_auc:.5f})')
    plt.plot(recalls, precisions, color='darkorange', linestyle='-', lw=2.5, label=f'Model C (Tx + Top-6 Behavior) (PR-AUC = {pr_auc:.5f})')
    plt.axvline(x=r_opt, color='red', linestyle=':', alpha=0.8, label=f'Model C Opt F1 Recall = {r_opt:.3f}')
    plt.axhline(y=p_opt, color='red', linestyle=':', alpha=0.8, label=f'Model C Opt F1 Precision = {p_opt:.3f}')
    plt.xlabel('Recall')
    plt.ylabel('Precision')
    plt.title('Comparative Precision-Recall Curves (Model A vs B vs C)')
    ...
    print("\n[7/7] Writing Model C evaluation report...")
    ...
    report_content = f"""# Phase 4B: LightGBM Transaction + Top Behavioral Model Evaluation Report
...
| :--- | :---: | :---: | :---: | :---: |
| **PR-AUC (Average Precision)** | `{b_pr_auc:.5f}` | `{mb_pr_auc:.5f}` | `{pr_auc:.5f}` | `{pr_auc - b_pr_auc:+.5f}` |
| **ROC-AUC** | `{b_roc_auc:.5f}` | `{mb_roc_auc:.5f}` | `{roc_auc:.5f}` | `{roc_auc - b_roc_auc:+.5f}` |
| **F1 @ 0.50 (Reference)** | `{bf_05:.5f}` | `{mbf_05:.5f}` | `{f_05:.5f}` | `{f_05 - bf_05:+.5f}` |
| **Precision @ 0.50** | `{bp_05:.5f}` | `{mbp_05:.5f}` | `{p_05:.5f}` | `{p_05 - bp_05:+.5f}` |
| **Recall @ 0.50** | `{br_05:.5f}` | `{mbr_05:.5f}` | `{r_05:.5f}` | `{r_05 - br_05:+.5f}` |
| **Optimal F1-Score** | `{b_best_f1:.5f}` | `{mb_best_f1:.5f}` | `{best_f1:.5f}` | `{best_f1 - b_best_f1:+.5f}` |
| **Optimal Threshold** | `{b_best_threshold:.4f}` | `{mb_best_threshold:.4f}` | `{best_threshold:.4f}` | `{best_threshold - b_best_threshold:+.4f}` |
| **FPR @ Optimal Threshold** | `{bfpr_opt:.5f}` | `{mbfpr_opt:.5f}` | `{fpr_opt:.5f}` | `{fpr_opt - bfpr_opt:+.5f}` |
...
* **Model B (TN / FP / FN / TP)**: {mbtn_05:,} / {mbfp_05:,} / {mbfn_05:,} / {mbtp_05:,} (Values from Model B Predictions)
* **Model C (TN / FP / FN / TP)**: {tn_05:,} / {fp_05:,} / {fn_05:,} / {tp_05:,}

### Optimal Threshold
* **Model A (TN / FP / FN / TP @ {b_best_threshold:.4f})**: {btn_opt:,} / {bfp_opt:,} / {bfn_opt:,} / {btp_opt:,}
* **Model B (TN / FP / FN / TP @ {mb_best_threshold:.4f})**: {mbtn_opt:,} / {mbfp_opt:,} / {mbfn_opt:,} / {mbtp_opt:,}
* **Model C (TN / FP / FN / TP @ {best_threshold:.4f})**: {tn_opt:,} / {fp_opt:,} / {fn_opt:,} / {tp_opt:,}
...
"""
    ...
    print(f"Saved behavioral evaluation report to: {report_path}")

    elapsed = time.time() - start_time
    print("\n" + "=" * 70)
    print("MODEL C TRAINING AND COMPARATIVE EVALUATION COMPLETED")
    print("=" * 70)
