...
├── experiments/              # Modeling & Stacking Experiments
│   ├── train_transaction_baseline.py # Model A (Baseline LightGBM)
│   ├── train_behavioral_model.py # Model B (LGBM Tx + All Behavioral)
│   ├── train_top_behavioral_model.py # Model C (LGBM Tx + Top Behavioral)
│   ├── train_behavior_only_models.py # Standalone LGBM/XGB/CatBoost benchmarks
│   ├── train_risk_fusion.py  # Stacking and linear fusion logic-gates
│   ├── train_historical_model.py # Model D (LGBM Tx + Historical - Champion)
│   ├── train_historical_xgb.py # Model E (XGBoost Tx + Historical)
│   ├── train_combined_model.py # Model F (LGBM Combined Tx + Hist + Behav)
│   └── analyze_behavioral.py # Behavioral statistics and metrics audit
